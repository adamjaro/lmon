...
//
// Version 2 with assembly of towers in z plane tilted along x and y
//_____________________________________________________________________________
...
//_____________________________________________________________________________
WScFiZXv2::WScFiZXv2(const G4String& nam, GeoParser *, G4LogicalVolume *top) : Detector(),
  G4VSensitiveDetector(nam), fNam(nam), fCheckOverlaps(false) {

  G4cout << "  WScFiZXv2: " << fNam << G4endl;
  ...
  G4double rotx = 10.0*degree;
  G4double roty = 1.0*degree;

  G4RotationMatrix *rmx = new G4RotationMatrix(CLHEP::HepRotationX(rotx)); 
  G4RotationMatrix *rmy = new G4RotationMatrix(CLHEP::HepRotationY(roty)); 
  G4RotationMatrix *rxy = new G4RotationMatrix((*rmx) * (*rmy));
  ...
  for(G4int ix=0; ix<nxy; ix++) {
    for(G4int iy=0; iy<nxy; iy++) {
      ...
      new G4PVPlacement(rxy, G4ThreeVector(xpos, ypos, 0), towv, towv->GetName(), modv, false, tcnt++);
    }
  }
...
//_____________________________________________________________________________
G4LogicalVolume* WScFiZXv2::MakeTower(G4double calorSizeXY, G4double calorEMZ) {
...
//_____________________________________________________________________________
G4bool WScFiZXv2::ProcessHits(G4Step *step, G4TouchableHistory*) {
...
//_____________________________________________________________________________
G4double WScFiZXv2::BirksCorrectedEnergyDeposit(G4Step *step) {
...
//_____________________________________________________________________________
void WScFiZXv2::CreateOutput(TTree *tree) {
